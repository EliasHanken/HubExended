...
public class ConfigUtil {
    ...
    public static void init(HubExtended plugin) {
        try {
            ...
            for (Player all : Bukkit.getOnlinePlayers()) {
                if (all.isOp()) {
                    all.sendMessage("\n§8[§cSWKillEffect§8] §cconfig.yml の読み込み中にエラーが発生しました (" + e.getLocalizedMessage() + ")");
                    all.sendMessage("§8[§cSWKillEffect§8] §cコンソールを見て、 config.yml を修正してください\n");
                }
            }
            e.printStackTrace();
        }
